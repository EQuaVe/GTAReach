...
edge:P${p}:idle:acq:take${LEFT}{{provided:;do: x${p};}}
edge:P${p}:acq:idle:release${LEFT}{{provided: x${p}>=${TIMEOUT};}}
edge:P${p}:acq:eat:take${RIGHT}{{provided: x${p}<=${TIMEOUT} ; do: x${p};}}
edge:P${p}:eat:rel:release${RIGHT}{{provided: x${p}>=${EAT} ; do: x${p};}}
edge:P${p}:rel:idle:release${LEFT}{{provided:;}}
"
done
