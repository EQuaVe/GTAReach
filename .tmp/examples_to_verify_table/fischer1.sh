...
edge:P$pid:A:req:tau{{provided:id==0 ; do:x$pid;}}
edge:P$pid:req:wait:tau{{provided:x$pid<=$K ; do:x$pid,id=$pid;}}
edge:P$pid:wait:req:tau{{provided:id==0 ; do:x$pid;}}
edge:P$pid:wait:cs:tau{{provided:x$pid>$K&&id==$pid;}}
edge:P$pid:cs:A:tau{{provided:;do:id;}}
"
done
