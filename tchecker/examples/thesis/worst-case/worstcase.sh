...
# Automaton triggering tasks (worst case releases)
...
for i in `seq 1 $NTASKS`; do
    ...
    echo "location:taskautomaton:tasktemp${i}{}"
done
echo "edge:taskautomaton:init:released1:release1{provided: zerotime<=0}"
if [ ${NTASKS} -gt 1 ]; then
    for i in `seq 1 $(($NTASKS - 1))`; do
        echo "edge:taskautomaton:released${i}:released$((${i}+1)):release$((${i}+1)){provided: zerotime<=0}"
    done
fi
for i in `seq 1 ${NTASKS}`; do
    echo "edge:taskautomaton:released${NTASKS}:tasktemp${i}:done${i}{do: zerotime=0}"
    echo "edge:taskautomaton:tasktemp${i}:released${NTASKS}:release${i}{provided: zerotime<=0}"
done
...
for i in `seq 1 $NTASKS`; do
    SYNCEXPR="sync:task${i}@done${i}:scheduler@done${i}:taskautomaton@done${i}"
    for j in `seq 1 $NTASKS`; do
        if [ $i != $j ]; then
