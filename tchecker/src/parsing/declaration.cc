...
attributes_t::attributes_t(tchecker::parsing::attributes_t && attr) : _attr(std::move(attr._attr)) {}

attributes_t::~attributes_t() { clear(); }
...
void attributes_t::insert(tchecker::parsing::attr_t const * attr) { _attr.insert({attr->key(), attr}); }
...
clock_declaration_t::clock_declaration_t(std::string const & name, unsigned int size, tchecker::parsing::attributes_t && attr, unsigned int type, std::string const & event_name)
    : tchecker::parsing::inner_declaration_t(std::move(attr)), _name(name), _size(size), _clock_type(type), _event_name(event_name)
{
  if (_name.empty())
  ...
  // std::cout << "Added clock clock_declaration_t " << name << std::endl;
  //ani:-100 changes
  // if (_event_name.empty() && _clock_type>0)
  //   throw std::invalid_argument("clock declaration for an event clock has empty name");
}
...
tchecker::parsing::declaration_t * clock_declaration_t::do_clone() const
{
  ...
  //ani:-100
  return new tchecker::parsing::clock_declaration_t(_name, _size, std::move(attr), _clock_type, _event_name);
}
...
{
  // std::cout << "PROCESS DECL " << name << std::endl; 
  if (_name.empty())
...
{
  // std::cout << "declaring location " << name << std::endl;
  if (_name.empty())
...
{
  // std::cout << "declaring edge \n";
  // src.do_output(std::cout);
  // std::cout << " -> ";
  // tgt.do_output(std::cout);
  // std::cout << " :: ";
  // event.do_output(std::cout);
  // std::cout << std::endl;
...
{
  os << "edge:" << _process.name() << ":" << _src.name() << ":" << _tgt.name() << ":";
  os << _event.name() << _attr;
  return os;
}
...
public:
   ...
   */
  system_duplicator_t(tchecker::parsing::system_declaration_t & sysdecl) : _sysdecl(sysdecl) {}
...
void system_declaration_t::do_visit(tchecker::parsing::declaration_visitor_t & v) const { v.visit(*this); }
